...
import { createDynamicFormSchema, DynamicFormData } from "@/schema/surveySchema";
...
export default function MultiStepSurvey({ survey }: MultiStepSurveyProps) {
  ...
  const formSchema = useMemo(() => createDynamicFormSchema(survey.questions), [survey.questions]);
  ...
  const handleMultiChoiceChange = (fieldName: string, option: string, checked: boolean) => {
  ...
  const onSubmit = async (data: SurveyFormData) => {
    ...
    try {
      ...
      survey.questions.forEach((question, index) => {
        ...
        console.log(`Processing ${question.question_key}: ${JSON.stringify(answer)} (type: ${typeof answer})`);
      });
      ...
      console.log("board_state_name value:", answers.board_state_name, "type:", typeof answers.board_state_name);
              ...
              {question.options?.map((option, optionIndex: number) => {
                ...
                return (
                  <div key={optionIndex} className="flex items-center space-x-2">
                    <RadioGroupItem value={value} id={`${fieldName}-${optionIndex}`} />
                    <Label 
              ...
              return (
                <div key={optionIndex} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    checked={(multiChoiceSelections[fieldName] || []).includes(optionValue)}
                    onCheckedChange={(checked) => 
                      handleMultiChoiceChange(fieldName, optionValue, checked === true)
                    }
                  />
                  <Label 
                    htmlFor={`${fieldName}-${optionIndex}`} 
